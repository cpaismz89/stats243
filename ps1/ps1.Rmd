...
```{r, engine = 'bash', eval = FALSE, Q2a12}
...
\subsubsection{b) Bash Function}
  The main strategy for coding the function consists of creating a .sh file called \textbf{Faofunction.sh} where we will define our function, allowing us to call it after invoking the .sh file from the command line, avoiding the declaration of the function within the console. Inside this file, we start by defining a regular expression that will allow us to test if the input given by the user is a valid integer or not. Then, we detect all possible cases using conditional clauses (\textit{if}) to check if the input given by the user is not valid: non-numeric and/or the presence of more/less than one value. Using the same approach, we include a help message using the -h or --help flags, indicating the usage of the function and its description. 
...
\begin{itemize}
  \item[4.1] If all validations are passed (user gives a valid input), the file associated with the given product ID is downloaded as a .zip.
\end{itemize}

```{r, engine = 'bash', eval = FALSE, Q2b3}
...
```{r, engine = 'bash', eval = FALSE, Q2b32}
# Else, run the function
else										
    # Download the file in .zip format 
...
\subsection{Problem 4 code}
  This is the code used for generating the next page of this report.
  We start with the introduction, including the two r-inline functions for generating the years:
```{r, engine ='R', eval = FALSE, results = 'asis'} 
...
````r
`r ''````{r, engine='R', fig.width=3, fig.height=4} 
hist(LakeHuron)
```
````
...
show a histogram of the lake levels for the period `r 1876 - 1.` to `r 1000 + 72 + 900`


```{r, engine = 'R', fig.width = 3, fig.height = 4, P1}
hist(LakeHuron)
```
...
\subsection{Problem 3}
\subsubsection{a) Download files by extension from url }

```{r, engine = 'bash', eval = FALSE, AP31}
#!/bin/sh
# Get index.html file without output
wget -q https://www1.ncdc.noaa.gov/pub/data/ghcn/daily/			
