...
read_chunk('CitesScholar.R')
read_chunk('CreateDataFrame_subset.R')
read_chunk('CreateDataFrame.R')
read_chunk('DownloadfromScholar.R')
read_chunk('test_CreateDataFrame_subset.R')
read_chunk('test_CitesScholar.R')
@
...
\section*{Problem 2 \footnote{All results are in the Appendix section at the end of the document}} 
...
<<echo=TRUE, eval=FALSE, cache=FALSE>>=    
  ...
  if (length(Titles) <= 0){
    stop(paste("No Titles: There is no author's citation page inside Google Scholar",
...
setwd("C:/Users/chile/ps2/tests/")

# Invoking the test
test_file("test_CitesScholar.R")
@  
  
  Based on the output we can see that we get a series of warnings regarding the end of the html files we are using for testing. This is due to the fact that these files do not end with a $\backslash$ n or a $\backslash$ r $\backslash$ n characters and thus, the system outputs the corresponding warning. Then, we can easily check that all our tests for checking the input valid formats (when no valid ones were provided) ended as expected: stopping the function and displaying an error message to the user regarding the structure of a valid input argument. On the other hand, tests for checking the output types and their format when a valid input name is given are passed without any errors, as expected.  
...
test_that("Types and lengths are consistent when using a known html file", {
  ...
  FinalDF <- CreateDataFrame_subset(htmlParse(readLines("~/ps2/tests/
                                                        qc6CJjYAAAAJ.html")))
...
<<CitesScholar-ext, echo=TRUE, eval=TRUE, cache=FALSE>>=
@
...
<<DownloadfromScholar-ext, echo=TRUE, eval=TRUE, cache=FALSE>>=
@
...
<<CreateDataFramesubset-ext, echo=TRUE, eval=TRUE, cache=FALSE>>=
@

\subsubsection*{Run the code}
<<echo=TRUE, eval=TRUE, cache=FALSE, size="normalsize">>=
# Call the function and create a DataFrame from the author's website
ResultsList <- CitesScholar("Geoffrey Hinton")
...
<<echo=TRUE, eval=TRUE, cache=FALSE, size="small">>=
# Printing a summary using the apply function (``pretty'' output)
apply(SubsetScholarDF, 2, function(x) 
  if (length(x) > 15 && is.na(str_match(x, "^[[:digit:]]"))) {
    paste(substr(x, start = 0, stop = 15), "...", sep = "")
...
\subsubsection*{c.1) CitesScholar Tests}
<<testCitesScholar-ext, echo=TRUE, eval=FALSE, cache=FALSE>>=
@
...
\newpage
\subsubsection*{c.2) CreateDataFrame\_subset Tests}
<<testCreateDataFramesubset-ext, echo=TRUE, eval=FALSE, cache=FALSE>>=
@
...
<<echo=TRUE, eval=FALSE, cache=FALSE, size="normalsize">>=  
# Downloading the files (using wget for simplicity): valid, non-existent and non-valid
system('bash -c "wget -q -O qc6CJjYAAAAJ.html "https://scholar.google.com/"\
"citations?user=qc6CJjYAAAAJ&hl=es&oi=ao""')
system('bash -c "wget -q -O nonexistent.html "https://scholar.google.com/scholar?"\
...
<<CreateDataFrame-ext, echo=TRUE, eval=TRUE, cache=FALSE>>=
@
