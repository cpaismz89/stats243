...
recession = lines.filter(find).repartition(480) # 18 minutes for full
recession.count() # 433k observations for full dataset
...
counts = recession.map(stratify).reduceByKey(add) # 5 minutes
...
outputDir = '/global/home/users/cpaismz/recessionFull'
