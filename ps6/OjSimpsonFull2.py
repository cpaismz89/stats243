...
# Find the expression
def find(line, regex = "O._J._Simpson", language = None):
	vals = line.split(' ')
	if len(vals) < 6:
		return(False)
...
simpson = lines.filter(find).repartition(480) # 18 minutes for full
simpson.count() # 433k observations for full dataset
...
counts = simpson.map(stratify).reduceByKey(add) # 5 minutes
...
outputDir = '/global/home/users/cpaismz/OJSimpsonFull2'
