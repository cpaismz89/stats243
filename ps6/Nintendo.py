...
# Find the expression
def find(line, regex = "Nintendo", language = None):
	vals = line.split(' ')
	if len(vals) < 6:
		return(False)
...
nintendo = lines.filter(find).repartition(480) # 18 minutes for full
nintendo.count() # 433k observations for full dataset
...
counts = nintendo.map(stratify).reduceByKey(add) # 5 minutes
...
outputDir = '/global/home/users/cpaismz/nintendofull'
