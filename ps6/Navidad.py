...
# Find the expression
def find(line, regex = "Navidad", language = None):
	vals = line.split(' ')
	if len(vals) < 6:
		return(False)
...
navidad = lines.filter(find).repartition(480) # 18 minutes for full
navidad.count() # 433k observations for full dataset
...
counts = navidad.map(stratify).reduceByKey(add) # 5 minutes
...
outputDir = '/global/home/users/cpaismz/NavidadFull'
