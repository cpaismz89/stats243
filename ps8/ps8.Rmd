...
Looking at the plots, we can easily notice that the distribution with the heavier tail is the exponential distribution as we expected since we know (from our background and classes) that this is a specific characteristic of this distribution (among other relevant characteristics/properties) and it is usually used for simulating lifetimes and other real-world phenomena. Therefore, we will expect better estimation results using it as the sampling distribution for the importance sampling method.
...
From the previous results we can easily check that the mean value estimation is very accurate but in this case, the variance estimation surpass the true value of the distribution variance due to the fact that the Pareto distribution is not as good as the exponential distribution for applying the importance sampling methodology: it does not satisfy the assumption of being a heavy tail distribution. Therefore, we are not able to reduce the variance of the sampling.
...
\begin{itemize}
  \item[i)] A grid for the components $X1$ and $X2$ is generated in order to be able to generate a surface. These vectors are manipulated in order to be able to create a DataFrame containing all the components (each row is a vector) needed to pass as arguments of the helicoidal function. 
  \item[ii)] Using this DataFrame, a simple $apply()$ call using the helicoidal function as the main input will allow us to evaluate each row/vector with the function, obtaining a new vector of $z$ values.
...
ContourPlot <- function(X3=0, color=TRUE){ 
  ...
  # Apply the helicoidal function and transform output to a matrix (surface)
  ...
  if (color == TRUE){
    fc <- filled.contour(x = X1,
                       ...
                       xlab = "X1",
                       ylab = "X2",
                       main = paste0("Helicoidal contour X3 = ", X3[1]),
...
```{r Helplot, echo=TRUE, eval=TRUE, cache=TRUE, fig.align="center", fig.cap="Different contour plots from the Helicoidal function (fixed $X3 = 0$ value)", fig.height=6, fig.width=7}
# Set working directory
setwd("C:/Users/chile/Desktop/Stats243/HW/HW8/")
...
```{r Helplot2, echo=TRUE, eval=TRUE, cache=TRUE, fig.align="center", fig.cap="Different contour plots from the Helicoidal function (fixed $X3= 1$ value)", fig.height=6, fig.width=7}
# Set working directory
setwd("C:/Users/chile/Desktop/Stats243/HW/HW8/")
...
```{r Helplot3, echo=TRUE, eval=TRUE, cache=TRUE, fig.align="center", fig.cap="Different contour plots from the Helicoidal function (fixed $X3= 10$ value)", fig.height=6, fig.width=7}
# Set working directory
setwd("C:/Users/chile/Desktop/Stats243/HW/HW8/")
...
```{r Helplot4, echo=TRUE, eval=TRUE, cache=TRUE, fig.align="center", fig.cap="Different contour plots from the Helicoidal function (fixed $X3= 1e3$ value)", fig.height=6, fig.width=7}
# Set working directory
setwd("C:/Users/chile/Desktop/Stats243/HW/HW8/")
...
```{r Helplot5, echo=TRUE, eval=TRUE, cache=TRUE, fig.align="center", fig.cap="Different contour plots from the Helicoidal function (fixed $X3= 1e4$ value)", fig.height=6, fig.width=7}
# Set working directory
setwd("C:/Users/chile/Desktop/Stats243/HW/HW8/")
...
```{r Helplot6, echo=TRUE, eval=TRUE, cache=TRUE, fig.align="center", fig.cap="Different contour plots from the Helicoidal function (fixed $X3 = 1e5$ value)", fig.height=6, fig.width=7}
# Set working directory
setwd("C:/Users/chile/Desktop/Stats243/HW/HW8/")
...
```{r Helplot7, echo=TRUE, eval=TRUE, cache=TRUE, fig.align="center", fig.cap="Different contour plots from the Helicoidal function (fixed $X3 = 1e6$ value)", fig.height=6, fig.width=7}
# Set working directory
setwd("C:/Users/chile/Desktop/Stats243/HW/HW8/")
...
```{r Helplot8, echo=TRUE, eval=TRUE, cache=TRUE, fig.align="center", fig.cap="Different contour plots from the Helicoidal function (fixed $X3 = -1e4$ value, no color example)", fig.height=6, fig.width=7}
# Set working directory
setwd("C:/Users/chile/Desktop/Stats243/HW/HW8/")
...
```{r Helplot9, echo=TRUE, eval=TRUE, cache=TRUE, fig.align="center", fig.cap="Different contour plots from the Helicoidal function (fixed $X3 = -1e6$ value, no color example)", fig.height=6, fig.width=7}
# Set working directory
setwd("C:/Users/chile/Desktop/Stats243/HW/HW8/")
...
In this section, we would like to study the function in terms of its potential local minima points as well as try to find the global (if exists) minimum value of the helicoidal function. In order to perform this task, we will define a series of auxiliary functions that will act as wrappers of the already studied $optim()$, $optimx()$, and $nlm()$ functions in R. The main idea and strategy will be the following:
...
\begin{itemize}
  \item[1.] We first define our $OptF()$ auxiliary function that will take a starting point $x_0$ given by the user and will optimize the Helicoidal function. Once the new point and new objective value are obtained, we check if the current solution is an optimal point or a local minimum by re-optimizing the function from that point. We continue with this fashion until the solution converges (no improvement is obtained) to a certain value.  
\end{itemize}
...
Based on the results, we can clearly see that again, we have a great convergence of the solutions toward the vector $(1,0,0)$ where the value of the objective function is $0$. Looking at the helicoidal function defined in $ps8.R$, we can clearly see that the final objective function consists of a summation of quadratic terms and thus, all of them are $\geq 0$. Therefore, it is possible to conclude that the optimal (global minimum) objective value of the function under study is $0$, reached at the point $(1,0,0)$, indicating us that the tested optimization methods tend to have a great convergence toward the global optimal solution of the function.
...
\begin{eqnarray}
	Q(\theta / \theta_t) &=& C - n \log(\sigma) - \dfrac{1}{2 \sigma^{2}} \left( \sum_{i = 1}^{n} {(\delta_i - x_{i}^{'}\beta)}^{2} + \mathbf{Var}[z_j]\right) 	
\end{eqnarray} 
...
\begin{eqnarray}
	\dfrac{\partial Q(\theta / \theta_t)}{\partial \sigma} &=& - \dfrac{n}{\sigma} + \dfrac{1}{\sigma^{3}} \left( \sum_{i=1}^{m} {(y_i - x_{i}^{'}\beta)}^{2} + \sum_{i=m+1}^{n} {(z_j - x_{j}^{'}\beta)}^{2}  + \sum_{i=m+1}^{n} \mathbf{Var}[z_j] \right) \\
	&=& 0
\end{eqnarray}

Solving for $\sigma$ we get:
\begin{eqnarray}
	{\sigma^2}^{*} &=& \dfrac{1}{n}  \left( \sum_{i=1}^{m} {(y_i - x_{i}^{'}\beta)}^{2} + \sum_{i=m+1}^{n} {(z_j - x_{j}^{'}\beta)}^{2}  + \sum_{i=m+1}^{n} \mathbf{Var}[z_j] \right)
\end{eqnarray}
